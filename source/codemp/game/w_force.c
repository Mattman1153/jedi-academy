...
int ForcePowerUsableOn(gentity_t *attacker, gentity_t *other, forcePowers_t forcePower)
{
	...
	// MJN - Added so you cant use force on sleeping clients
	if (other && other->client && M_isSleeping(other))
	{
		return 0;
	}

	// MJN - Added so you cant use force on protected clients
	if (other && other->client && M_isProtected(other))
	{
		return 0;
	}
	...
	// MJN - Possible Full Force Duel implementation.
...
{ //called on a regular interval to regenerate force power.
	...
	else if( M_isEmpowered(self) ){
		self->client->ps.fd.forcePower += 10; // regenerate 10 times as fast.
	}
	else
	{ //otherwise, just 1
...
void ForceTeamHeal( gentity_t *self )
{
	...
	{
		// MJN - added for empowered users
		if( M_isEmpowered(self)){
			radius *= 10;
		}
		else{
			radius *= 2;
			...
			if (ForcePowerUsableOn(self, traceEnt, FP_LIGHTNING))
			{
				...
				{//2-handed lightning
					...
					if(M_isEmpowered(self)){
						...
						dmg *= 4; // MJN - Powerrrrrr....Unlimited Power!
	...
	if ( traceEnt && traceEnt->takedamage )
	{
		...
		{//an enemy or object
			...
			if (ForcePowerUsableOn(self, traceEnt, FP_DRAIN))
			{
				...
				{
					// MJN - Stronger for empowered users.
					if(M_isEmpowered(self)){
...
void ForceThrow( gentity_t *self, qboolean pull )
{
	...
	if (M_isEmpowered(self)) // MJN - Empowerment gives super radius.
	{
		visionArc = 350;
	}
	...
	for ( e = 0 ; e < numListedEntities ; e++ ) 
	{
		...
		// MJN - Empowered can only push other Empowered clients.
		if( ( ent->client && M_isEmpowered(ent) ) && 
				( self->client && !M_isEmpowered(self) ) )
		{
			continue;	
		}
		...
		for ( x = 0; x < ent_count; x++ )
		{
			...
			{//FIXME: make enemy jedi able to hunker down and resist this?
				...
				if (push_list[x]->client)
				{
					...
					// MJN - empower should have equal strength.  Escape a force grip if we're in one
					if ((self->client->ps.fd.forceGripBeingGripped > level.time) && M_isEmpowered(self) )
					{ //force the enemy to stop gripping me if I managed to push him
						if (push_list[x]->client->ps.fd.forceGripEntityNum == self->s.number && 
							!M_isProtected(push_list[x]))
						{
					...
					} // MJN - Only escape a force grip if we're in one and the client is not empowered.
					else if (self->client->ps.fd.forceGripBeingGripped > level.time)
					{ //force the enemy to stop gripping me if I managed to push him
						if (push_list[x]->client->ps.fd.forceGripEntityNum == self->s.number && 
							(!M_isEmpowered(push_list[x]) || !M_isProtected(push_list[x])))
						{
							if (modPowerLevel >= push_list[x]->client->ps.fd.forcePowerLevel[FP_GRIP])
							{ //only break the grip if our push/pull level is >= their grip level
								WP_ForcePowerStop(push_list[x], FP_GRIP);
...
void DoGripAction(gentity_t *self, forcePowers_t forcePower)
{
	...
	if (gripLevel == FORCE_LEVEL_2)
	{
		...
		else if ((level.time - gripEnt->client->ps.fd.forceGripStarted) > 4000 && !M_isEmpowered(self))
		{
			WP_ForcePowerStop(self, forcePower);
		}
		return;
	...
	case FP_GRIP:
		...
		if (self->client->ps.fd.forceGripEntityNum != ENTITYNUM_NONE)
		{
			if (!(self->client->ps.fd.forcePowersActive & (1 << FP_GRIP)))
			{
				...
				// MJN - Since we are more powerful when Empowered, we don't want our force to drain as quickly.
				if(M_isEmpowered(self)){
					BG_ForcePowerDrain( &self->client->ps, FP_GRIP, 10 );
				}else{
