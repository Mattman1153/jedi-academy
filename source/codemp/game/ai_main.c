...
int ScanForEnemies(bot_state_t *bs)
{
	...
	// MJN - added check for JM mode as we don't want them to do this when we're empowered.
	if (bs->currentEnemy && bs->currentEnemy->client &&
		bs->currentEnemy->client->ps.isJediMaster && g_gametype.integer == GT_JEDIMASTER)
	...
	while (i <= MAX_CLIENTS)
	{
		if (i != bs->client && g_entities[i].client && !OnSameTeam(&g_entities[bs->client], &g_entities[i]) && PassStandardEnemyChecks(bs, &g_entities[i]) && BotPVSCheck(g_entities[i].client->ps.origin, bs->eye) && PassLovedOneCheck(bs, &g_entities[i]))
		{
			...
			vectoangles(a, a);
			// MJN - Added check for JM mode.
			if (g_entities[i].client->ps.isJediMaster && g_gametype.integer == GT_JEDIMASTER)
				...
				if (BotMindTricked(bs->client, i))
				{
					if (distcheck < 256 || (level.time - g_entities[i].client->dangerTime) < 100)
					{
						...
						{ //if we have an enemy, only switch to closer if he is 128+ closer to avoid flipping out
							// MJN - Added check for JM mode.
							if (!noAttackNonJM || (g_entities[i].client->ps.isJediMaster && g_gametype.integer == GT_JEDIMASTER))
							{
								closest = distcheck;
								bestindex = i;
							}
...
void StandardBotAI(bot_state_t *bs, float thinktime)
{
	...
	{
		// MJN - We don't want them to leave their guard down during duel or powerduel gametypes.
		if (g_gametype.integer != GT_DUEL && g_gametype.integer != GT_POWERDUEL){
				...
				bs->currentEnemy->client->ps.saberHolstered)
			{
				...
				// MJN - Fix - added "!bs->cur_ps.duelInProgress" and "!bs->currentEnemy->client->ps.fd.forcePowersActive" 
				// so you cannot lame bots when in a private duel with them or using force on them before challenging.
...
int BotAISetup( int restart ) {
	...
	trap_Cvar_Register(&bot_honorableduelacceptance, "bot_honorableduelacceptance", "1", 
		CVAR_ARCHIVE | CVAR_NORESTART /*CVAR_CHEAT*/); // MJN - Default enabled and Not Cheat Protected.
