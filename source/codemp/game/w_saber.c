...
// MJN - Commented out, we no longer need this with the advanced saber system.
...
// MJN - #define SABER_NONATTACK_DAMAGE 1
// Made the damage be controlled by g_mSaberDMGIdle.integer instead.  If you want to change it back, just replace
// g_mSaberDMGIdle.integer with SABER_NONATTACK_DAMAGE and uncomment the define above.
	...
	for ( i = 0; i < numVictims; i++ )
	{
		...
		if ( te )
		{
			...
			else
			{
				...
				{
					if (totalDmg[i] > g_mSaberDMGIdle.integer)
...
static GAME_INLINE qboolean CheckSaberDamage(gentity_t *self, int rSaberNum, int rBladeNum, vec3_t saberStart, vec3_t saberEnd, qboolean doInterpolate, int trMask, qboolean extrapolate )
{
	...
	{ //this animation is that of the last attack movement, and so it should do full damage
		...
		{
			dmg = g_mSaberDMGBlueNormal.integer; // MJN - SABER_HITDAMAGE;
				...
				self->client->ps.fd.saberAnimLevel == SS_DUAL)
			{
					...
					else
					{
						...
						{ //special kata move
							...
							{ //this is the nasty saber twirl, do big damage cause it makes you vulnerable
								...
								//MJN
								dmg = g_mSaberDMGDualKata.integer;
								//dmg = 90;
							}
							else
							{ //staff kata
								...
								// MJN 
								dmg = G_GetAttackDamage(self, g_mSaberDMGStaffKataMin.integer, g_mSaberDMGStaffKataMax.integer, 0.5f);
						...
						else
						{
							...
							dmg = G_GetAttackDamage(self, g_mSaberDMGSpecialMin.integer, g_mSaberDMGSpecialMax.integer, 0.5f);
				...
				{ //otherwise we'll ramp up to 70 I guess, for both dual and staff
					...
					//MJN
					dmg = G_GetAttackDamage(self, g_mSaberDMGMultiMin.integer, g_mSaberDMGMultiMax.integer, 0.5f);
			...
			else if (self->client->ps.fd.saberAnimLevel == 3)
			{
				...
				{
					//MJN
					dmg = G_GetAttackDamage(self, g_mSaberDMGRedNormalMin.integer, g_mSaberDMGRedNormalMax.integer, 0.5f);
				...
				{
					//MJN
					dmg = G_GetAttackDamage(self, g_mSaberDMGRedDFAMin.integer, g_mSaberDMGRedDFAMax.integer, 0.65f);
					//dmg = G_GetAttackDamage(self, 2, 180, 0.65f);
				}
				else if (inBackAttack)
				{
					//MJN
					dmg = G_GetAttackDamage(self, g_mSaberDMGRedBackMin.integer, g_mSaberDMGRedBackMax.integer, 0.5f);
					//dmg = G_GetAttackDamage(self, 2, 30, 0.5f); //can hit multiple times (and almost always does), so..
				}
				else
				{
					//MJN
					dmg = g_mSaberDMGRedNormal.integer;
					//dmg = 100;
				}
			}
			else if (self->client->ps.fd.saberAnimLevel == 2)
			{
				...
				{ //a well-timed hit with this can do a full 85
					...
					//MJN
					dmg = G_GetAttackDamage(self, g_mSaberDMGYellowOverheadMin.integer, g_mSaberDMGYellowOverheadMax.integer, 0.5f);
					//dmg = G_GetAttackDamage(self, 2, 80, 0.5f);
				}
				else if (inBackAttack)
				{
					//MJN
					dmg = G_GetAttackDamage(self, g_mSaberDMGYellowBackMin.integer, g_mSaberDMGYellowBackMax.integer, 0.5f);
					//dmg = G_GetAttackDamage(self, 2, 25, 0.5f);
				}
				else
				{
					//MJN
					dmg = g_mSaberDMGYellowNormal.integer;
					//dmg = 60;
				}
			}
			else if (self->client->ps.fd.saberAnimLevel == 1)
			{
				...
				{
					//MJN
					dmg = G_GetAttackDamage(self, g_mSaberDMGBlueLungeMin.integer, g_mSaberDMGBlueLungeMax.integer, 0.3f);
					//dmg = G_GetAttackDamage(self, 2, SABER_HITDAMAGE-5, 0.3f);
				}
				else if (inBackAttack)
				{
					//MJN
					dmg = G_GetAttackDamage(self, g_mSaberDMGBlueBackMin.integer, g_mSaberDMGBlueBackMax.integer, 0.5f);
					//dmg = G_GetAttackDamage(self, 2, 30, 0.5f);
				}
				else
				{
					//MJN
					dmg = g_mSaberDMGBlueNormal.integer;
					//dmg = SABER_HITDAMAGE;
				}
			}
	...
	if (BG_SaberInSpecial(self->client->ps.saberMove))
	{
		...
		else if (!inBackAttack)
		{
			...
			{ //do extra damage for special unblockables
				//MJN
				dmg += g_mSaberDMGRedDFAAmplifier.integer;
			...
			{
				//MJN
				dmg += g_mSaberDMGYellowOverheadAmplifier.integer;
				//dmg += 5; //ditto
			...
			{
				//MJN
				dmg += g_mSaberDMGBlueLungeAmplifier.integer;
			...
			{ //do a constant significant amount of damage but ramp up a little to the mid-point
				...
				//MJN
				dmg += g_mSaberDMGTwirlAmplifier.integer;
				//dmg += 10;
			}
			else
			{
				...
				{ //auto-ramp it I guess since it's a special we don't have a special case for.
					...
					dmg = G_GetAttackDamage(self, g_mSaberDMGAutoAmplifier.integer, g_mSaberDMGAutoAmplifier.integer+5, 0.5f);
	...
	if (dmg > g_mSaberDMGIdle.integer)
	{
		...
		{ //weaken it if an arm is broken
			...
			if (dmg <= g_mSaberDMGIdle.integer)
			{
				dmg = g_mSaberDMGIdle.integer+1;
			}
		}
	}
	// MJN - added check for JM mode.  We don't want you to have too much power!
	if (dmg > g_mSaberDMGIdle.integer && self->client->ps.isJediMaster && g_gametype.integer == GT_JEDIMASTER)
	{ //give the Jedi Master more saber attack power
		dmg *= 2;
	}

	// MJN - I'm not sure I want this, I will have to decide later.
	if (dmg > g_mSaberDMGIdle.integer && g_gametype.integer == GT_SIEGE &&
	...
	// MJN - Commented the whole block out; the code didn't do anything anyways.
	//if (g_gametype.integer == GT_POWERDUEL &&
	...
	// MJN
	// TODO: Implement a padawan mode where you can see the damage you deliver.
	// This could be a great way to train and know what really does the most damage.
			...
			*/
			...
			if ( !d_saberSPStyleDamage.integer )
			{
				...
				{ //for jedi using the saber, half the damage (this comes with the increased default dmg debounce time)
					...
					{ //unless siege..
						if (dmg > g_mSaberDMGIdle.integer && !unblockable)
						{ //don't reduce damage if it's only 1, or if this is an unblockable attack
							if (dmg == g_mSaberDMGBlueNormal.integer) // MJN - was SABER_HITDAMAGE
							{ //level 1 attack
								dmg *= 0.7;
							}
							else
...
blockStuff:
		...
		if (dmg > g_mSaberDMGIdle.integer && !unblockable && !otherUnblockable)
		{
			int lockFactor = g_saberLockFactor.integer;
		...
		else if (SaberAttacking(otherOwner) && dmg > g_mSaberDMGIdle.integer && !BG_SaberInSpecial(otherOwner->client->ps.saberMove) && !didOffense && !otherUnblockable)
		...
		if (d_saberGhoul2Collision.integer && !didDefense && dmg <= g_mSaberDMGIdle.integer && !otherUnblockable) //with perpoly, it looks pretty weird to have clash flares coming off the guy's face and whatnot
		{
			if (!PM_SaberInParry(otherOwner->client->ps.saberMove) &&
				!PM_SaberInBrokenParry(otherOwner->client->ps.saberMove) &&
				!BG_SaberInSpecial(otherOwner->client->ps.saberMove) &&
		...
		else if (!didDefense && dmg > g_mSaberDMGIdle.integer && !otherUnblockable) //if not more than idle damage, don't even bother blocking.
		{ //block
			if (!PM_SaberInParry(otherOwner->client->ps.saberMove) &&
				!PM_SaberInBrokenParry(otherOwner->client->ps.saberMove) &&
				!BG_SaberInSpecial(otherOwner->client->ps.saberMove) &&
...
// MJN - Commented out and replaced with g_mSaberDMGThrow.integer
// #define SABER_THROWN_HIT_DAMAGE 30

// MJN - This was not needed since it is not used.
		...
		{ //within range
			...
			{ //Slice them
				...
				{ //a good hit
					...
					if (saberOwner->client->ps.isJediMaster && g_gametype.integer == GT_JEDIMASTER)
...
qboolean saberKnockOutOfHand(gentity_t *saberent, gentity_t *saberOwner, vec3_t velocity)
{
	...
	saberent->damage = g_mSaberDMGThrow.integer; // MJN
...
// MJN - Commented out.  We don't use this function.
/*
