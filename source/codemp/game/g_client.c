...
team_t PickTeam( int ignoreClientNum ) {
	...
	//return TEAM_BLUE;
	return TEAM_SPECTATOR; // MJN - If all else fails, make them spectate
}
	...
	*p = 0;
	...
	while( 1 ) {
		...
		if( ch == Q_COLOR_ESCAPE ) {
			...
			if( ColorIndex(*in) == 0 && ( g_mAllowBlack.integer == 0 ) ) {
				in++;
				continue;
			}
...
//	char	blueTeam[MAX_INFO_STRING];
	...
	// MJN - Save the players IP for if we want to ban him >:)
	if( s[0] != 0 ){
		// strcpy( client->sess.clientIP, s );
		M_Cmd_SaveIP_f( client, s );
	}
	...
	// MJN - Added so bots names wont change if you add same bot
	if( !(ent->r.svFlags & SVF_BOT) ){

		// MJN - See if we need to find a new name because the one they chose is taken
	...
	if ( client->pers.connected == CON_CONNECTED ) {
		...
		{
			// MJN - Dont let them change their name too much
...
//	char		*areabits;
	...
	// Fix: ensiform
	if ( firstTime && G_CheckMaxConnections( value ) ) {
		return va("Maximum Connections Per IP: %d.", sv_maxConnections.integer );
	}
	// Fix: ET Source and ensiform
	...
	if ( !isBot && g_needpass.integer && (strcmp(Info_ValueForKey ( userinfo, "ip" ), "localhost") != 0)) {
		...
		// MJN - now we see who is allowed on the server by the settings and the supplied password.
		switch( g_mAdminLoginControl.integer ){
			
			case 1: // Admin Only Server, you need the admin pass to get in.
				if( strcmp( g_mAdminPassword.string, value) != 0 ){
				...
				break;
			case 2: // Knight Only Server, you need the knight pass to get in.
				if( strcmp( g_mKnightPassword.string, value) != 0 ){
					static char sTemp[1024];
					Q_strncpyz(sTemp, G_GetStringEdString("MP_SVGAME","INVALID_ESCAPE_TO_MAIN"), sizeof (sTemp) );
					return sTemp;// return "Invalid password"
				}
				break;
			case 3: // Admin / Knight Only Server, you need the admin or knight pass to get in.
				if( strcmp( g_mAdminPassword.string, value) && strcmp( g_mKnightPassword.string, value) != 0 ){
					static char sTemp[1024];
					Q_strncpyz(sTemp, G_GetStringEdString("MP_SVGAME","INVALID_ESCAPE_TO_MAIN"), sizeof (sTemp) );
					return sTemp;// return "Invalid password"
				}
				break;
			case 4: // Admins and people that have the g_password server only.
				if ( g_password.string[0] && Q_stricmp( g_password.string, "none" ) != 0 ){
					if ( strcmp( g_password.string, value) != 0 ){
						if( strcmp( g_mAdminPassword.string, value) != 0 ){
							static char sTemp[1024];
							Q_strncpyz(sTemp, G_GetStringEdString("MP_SVGAME","INVALID_ESCAPE_TO_MAIN"), sizeof (sTemp) );
							return sTemp;// return "Invalid password"
				...
				break;
			case 5: // Knights and people that have the g_password server only.
				if ( g_password.string[0] && Q_stricmp( g_password.string, "none" ) != 0 ){
					if ( strcmp( g_password.string, value) != 0 ){
						if( strcmp( g_mKnightPassword.string, value) != 0 ){
							static char sTemp[1024];
				...
				break;
			case 6: // Admins, Knights, and people that have the g_password server only.
				if ( g_password.string[0] && Q_stricmp( g_password.string, "none" ) != 0 ){
					if ( strcmp( g_password.string, value) != 0 ){
						if( strcmp( g_mAdminPassword.string, value) && strcmp( g_mKnightPassword.string, value) != 0 ){
							static char sTemp[1024];
				...
				break;
			default: // let them in if g_password is met.
				if ( g_password.string[0] && Q_stricmp( g_password.string, "none" ) != 0 ){
					if ( strcmp( g_password.string, value) != 0 ){
						static char sTemp[1024];
						Q_strncpyz(sTemp, G_GetStringEdString("MP_SVGAME","INVALID_ESCAPE_TO_MAIN"), sizeof (sTemp) );
...
//	areabits = client->areabits;
	...
	if( g_mAdminLoginControl.integer ){
		// MJN - Did we supply a password?
		value3 = Info_ValueForKey (userinfo, "password");

		// MJN - If I supplied the password as the admin pass, log me in as well.
		if (strcmp( g_mAdminPassword.string, value3) == 0){
			client->sess.loggedinadmin = qtrue;
			G_LogPrintf( "mlog_adminlogin: %i\n", clientNum );
		}
		else if (strcmp( g_mKnightPassword.string, value3) == 0){
			client->sess.loggedinknight = qtrue;
			G_LogPrintf( "mlog_knightlogin: %i\n", clientNum );
		}
	}

	//MJN - initialize the motd flag
	client->sess.startShowMOTD = qfalse;
	...
	if ( firstTime ) {
		...
		// MJN - Track IP's and names on Connect.
		value = Info_ValueForKey (userinfo, "ip");
		value2 = Info_ValueForKey (userinfo, "name");
		if( !isBot ){
			G_LogPrintf( "mlog_ClientConnect: %i '%s' -> '%s'\n", clientNum, value2, value );
		}
	}
...
//		client->areabits = G_Alloc( (trap_AAS_PointReachabilityAreaIndex( NULL ) + 7) / 8 );
	...
	// MJN - so it doesn't spam the client everytime you join into a duel.
	if (firstTime && (g_gametype.integer == GT_DUEL || g_gametype.integer == GT_POWERDUEL)){
		client->sess.startShowMOTD = qtrue;
...
void ClientBegin( int clientNum, qboolean allowTeamReset ) {
	...
	if ( client->sess.sessionTeam != TEAM_SPECTATOR ) {
		...
		if ( g_gametype.integer != GT_DUEL || g_gametype.integer == GT_POWERDUEL ) {
			...
			//MJN - Welcome Message
			M_DisplayWelcomeMessage(ent);
			
			//MJN - Show MOTD only in gametypes other than duel and powerduel
			//if (g_gametype.integer != GT_DUEL && g_gametype.integer != GT_POWERDUEL){
			if (client->sess.startShowMOTD == qtrue){
				//client->sess.startShowMOTD = qtrue;
				M_Svcmd_ShowMOTD_f(ent);
			}
		}
	}
...
//	savedAreaBits = client->areabits;
	...
	// MJN - Terminators spawn with 200 Jet Pack Fuel.
	if( M_isTerminator(ent) ){
		client->ps.jetpackFuel = 200;
	}
	else{//spawn with 100
		client->ps.jetpackFuel = 100;
	}
	// MJN - Terminators spawn with 200 Cloak Fuel.
	if( M_isTerminator(ent) ){
		client->ps.cloakFuel = 200;
	}
	else{//spawn with 100
...
//	client->areabits = savedAreaBits;
		...
		&& g_trueJedi.integer )
	{
		...
		if ( WP_HasForcePowers( &client->ps ) )
		{
			...
			if( g_mJvsT.integer ){
				// MJN - lets make sure they are not a terminator.
				if( M_isTerminator(ent)){
					M_Svcmd_UnTerminator_f( ent );
				}
				// MJN - client is now empowered.
				M_Svcmd_Empower_f( ent );
			}
		}
		else
		{//no force powers set
			...
			// MJN - Empowered VS. Terminator expansion
			if( g_mJvsT.integer ){
				// MJN - lets make sure they are not empowered.
				if( M_isEmpowered(ent) ){
					M_Svcmd_UnEmpower_f( ent );
				}
				// MJN - client is now a Terminator
				M_Svcmd_Terminator_f( ent );
			}
			else {
	...
	{//jediVmerc is incompatible with this gametype, turn it off!
		...
		trap_Cvar_Set( "g_mJvsT", "0" ); // MJN - This doesn't either
		if (g_gametype.integer == GT_HOLOCRON)
		{
		...
		else
		{
			...
			{
				// MJN - Perhaps we can melee here too?
				client->ps.stats[STAT_WEAPONS] |= ( 1 << WP_SABER );	//these are precached in g_items, ClearRegisteredItems()
			}
			else
...
*/
	...
	*/
	...
	// MJN - Update players stats such as empowered and terminator.
	// They can get reset when a player dies and respawns.
	if( g_mJvsT.integer && g_trueJedi.integer ){
		if( ent->client && ent->client->ps.trueJedi && M_isEmpowered(ent) ){
			M_Svcmd_Empower_f( ent );
		}
		else if( ent->client && ent->client->ps.trueNonJedi && M_isTerminator(ent) ){
			M_Svcmd_Terminator_f( ent );
		}
	}
	else{ // Just update players if they are empowered and terminator. 
		if( ent->client && M_isEmpowered(ent) ){
			M_Svcmd_Empower_f( ent );
		}else if( ent->client && M_isTerminator(ent) ){
			M_Svcmd_Terminator_f( ent );
		}
	}
	//MJN - Update players if they are protected
	if( ent->client && M_isProtected(ent) )
	{
		M_Svcmd_Protect_f( ent );
	}
...
void ClientDisconnect( int clientNum ) {
	...
	// MJN - Remove Ignore sessions.
	G_RemoveFromAllIgnoreLists( clientNum );
	// MJN - Clear Admin flags
	M_ClearAdminFlags(ent);
