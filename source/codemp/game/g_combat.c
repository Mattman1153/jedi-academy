...
void player_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath ) {
	...
	qboolean	wasEmpowered = qfalse;
	qboolean	wasTerminator = qfalse;
	int			sPMType = 0;
...
extern void RunEmplacedWeapon( gentity_t *ent, usercmd_t **ucmd );
	...
	*/
	...
	// MJN - added so you wont die
	if (self->client && self->client->ps.isJediMaster )
	{
		wasJediMaster = qtrue;
	}
	// MJN - added so you wont die
	if ( self->client && M_isEmpowered(self) )
	{
		wasEmpowered = qtrue;
	}

	// MJN - added so you wont die
	if (self->client && M_isTerminator(self) )
	{
		wasTerminator = qtrue;
	}
		...
		if ( attacker && attacker->client ){
			...
			switch( g_mDuelStats.integer ){
				case 1: //Private Logging
						trap_SendServerCommand( attacker-g_entities, va("print \"%s ^7defeated %s ^7in ^5%d ^7hits ^7with ^1%i ^7HP ^7and ^2%i ^7Shields Remaining.\n\"", 
							attacker->client->pers.netname, self->client->pers.netname, attacker->client->ps.persistant[PERS_HITS], attacker->client->ps.stats[STAT_HEALTH], attacker->client->ps.stats[STAT_ARMOR] ) );
						trap_SendServerCommand( self-g_entities, va("print \"%s ^7defeated %s ^7in ^5%d ^7hits ^7with ^1%i ^7HP ^7and ^2%i ^7Shields Remaining.\n\"", 
							attacker->client->pers.netname, self->client->pers.netname, attacker->client->ps.persistant[PERS_HITS], attacker->client->ps.stats[STAT_HEALTH], attacker->client->ps.stats[STAT_ARMOR] ) );
						break;
				case 2: //Public Logging
						trap_SendServerCommand( -1, va("print \"%s ^7defeated %s ^7in ^5%d ^7hits ^7with ^1%i ^7HP ^7and ^2%i ^7Shields Remaining.\n\"", 
							attacker->client->pers.netname, self->client->pers.netname, attacker->client->ps.persistant[PERS_HITS], attacker->client->ps.stats[STAT_HEALTH], attacker->client->ps.stats[STAT_ARMOR] ) );
						break;
				default: // MJN - Do it the basejk way, off
						break;
	...
	if (attacker && attacker->client) {
		...
		if (self->client && self->client->ps.isJediMaster && 
				g_gametype.integer == GT_JEDIMASTER) // MJN - Added check for JM mode.
		{ //killed ourself so return the saber to the original position
		  //(to avoid people jumping off ledges and making the saber
		  //unreachable for 60 seconds)
			ThrowSaberToAttacker(self, NULL);
...
//		self->client->ps.pm_flags |= PMF_UPDATE_ANIM;		// Make sure the pmove sets up the GHOUL2 anims.
		...
		// MJN - JediMaster, Empowererd, or Terminator
		if (wasJediMaster || wasEmpowered || wasTerminator)
		{
...
void G_Damage( gentity_t *targ, gentity_t *inflictor, gentity_t *attacker,
	...
	if ((g_trueJedi.integer || g_gametype.integer == GT_SIEGE) && client) 
