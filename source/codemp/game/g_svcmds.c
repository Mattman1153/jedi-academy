...
static void UpdateIPBans (void)
{
	...
	trap_FS_FOpenFile( g_mBanFile.string, &f, FS_WRITE );
	if ( !f )
	{
		trap_Printf( va("BAN Error: File %s cannot be opened for writing.\n", g_mBanFile.string) );
		return;
	}
...
// BAN -- same as above but we don't update the file!
...
void G_ProcessIPBans( void ) 
{
	...
	len = trap_FS_FOpenFile( g_mBanFile.string, &f, FS_READ );
	if ( !f )
	{
...
void Svcmd_ListIP_f (void)
{
	...
	int i;
		...
		G_Printf("Current Banned IP Address:\n");
		for (i = 0 ; i < numIPFilters ; i++)
		{
			if (ipFilters[i].compare == 0xffffffff)
				continue;
...
qboolean	ConsoleCommand( void ) {
	...
	// MJN - ShowMOTD
	if ( Q_stricmp (cmd, "mshowmotd") == 0 ){
		ExecCommandOnPlayers(M_Svcmd_ShowMOTD_f, "Usage: mshowmotd <playerid>\n", 2, NULL, NULL, NULL);
		return qtrue;
	}
	...
	/*if ( Q_stricmp (cmd, "micaruslist") == 0 ){
		M_Svcmd_JAR_IcarusList_f();
		return qtrue;
	}*/
...
*/
	...
	if (g_dedicated.integer) {
		...
		if (Q_stricmp (cmd, "say") == 0) {
			trap_SendServerCommand( -1, va("print \"JAE Server: %s\n\"", ConcatArgs(1) ) );
			return qtrue;
		}
		// MJN - Na, we don't want people seeing values of certain cvars.  So print to server console only!
		// everything else will also be printed as a say command
		// trap_SendServerCommand( -1, va("print \"server: %s\n\"", ConcatArgs(0) ) );
		G_Printf("JAE Server: %s\n", ConcatArgs(0));
		return qtrue;
	}
