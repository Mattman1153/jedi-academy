			...
			for( i = 0; i < level.maxclients; i++ ){
				if( g_entities[i].client->pers.connected ){
					...
					trap_SendServerCommand( ent->client->ps.clientNum, va( "print \"%i%10c^7%i.%i.%i.%i%10c^7%s%10c^7<^1%i^7/^2%i^7>%10c^7%i\n\"", 
						i, ' ', g_entities[i].client->sess.ip[0], g_entities[i].client->sess.ip[1], g_entities[i].client->sess.ip[2], g_entities[i].client->sess.ip[3], 
...
void M_Svcmd_KickBan_f ( gentity_t * targetplayer )
{
	...
	if( clientid != -1 ){
		...
		trap_SendConsoleCommand( EXEC_APPEND, va( "AddIP %i.%i.%i.%i", client->sess.ip[0], client->sess.ip[1], 
		...
		G_LogPrintf("mlog_kickban: IP: %i.%i.%i.%i was banned for user %s.\n", client->sess.ip[0], client->sess.ip[1], 
			client->sess.ip[2], client->sess.ip[3], targetplayer->client->pers.netname);
	}
	else{
...
void M_Svcmd_BanRange_f ( gentity_t * targetplayer )
{
	...
	if( clientid != -1 ){
		...
		if ( !Q_stricmp( range, "light" ) || !Q_stricmp( range, "l" ) ) {
			...
			// Add ip to banlist and then kick the player:
			trap_SendConsoleCommand( EXEC_APPEND, va( "AddIP %i.%i.%i.0", client->sess.ip[0], client->sess.ip[1], client->sess.ip[2] ) );
		
			// MJN - Prints ip mask banned!
			G_Printf( va( "IP Range: %i.%i.%i.0 banned!\n", client->sess.ip[0], client->sess.ip[1], client->sess.ip[2] ));
			G_LogPrintf("mlog_kickban: IP: %i.%i.%i.0 was banned for user %s.\n", client->sess.ip[0], client->sess.ip[1], 
				client->sess.ip[2], targetplayer->client->pers.netname);
		}
		else if ( !Q_stricmp( range, "medium" ) || !Q_stricmp( range, "m" ) ){
			
			// Add ip to banlist and then kick the player:
			trap_SendConsoleCommand( EXEC_APPEND, va( "AddIP %i.%i.0.0", client->sess.ip[0], client->sess.ip[1] ) );
			...
			// Add ip to banlist and then kick the player:
			trap_SendConsoleCommand( EXEC_APPEND, va( "AddIP %i.0.0.0", client->sess.ip[0] ) );
		
			// MJN - Prints ip mask banned!
			G_Printf( va( "IP Range: %i.0.0.0 banned!\n", client->sess.ip[0]));
			G_LogPrintf("mlog_kickban: IP: %i.0.0.0 was banned for user %s.\n", client->sess.ip[0], targetplayer->client->pers.netname);
		}
		else{
		...
		if( M_isAdmin(ent) || M_isKnight(ent)){
			...
			if (tr.entityNum < MAX_CLIENTS && tr.entityNum != ent->s.number){
				...
				if( Q_stricmp( cmd, "slapgun" ) == 0){
					...
					// Add IP to banlist and then kick the player:
					trap_SendConsoleCommand( EXEC_APPEND, va( "AddIP %i.%i.%i.%i", other->client->sess.ip[0], 
						other->client->sess.ip[1], other->client->sess.ip[2], other->client->sess.ip[3] ) );
...
void M_Svcmd_Slap_f( gentity_t * targetplayer )
{
	...
	powerXY = trap_Cvar_VariableIntegerValue( "g_mSlapPowerXY" );
	powerZ = trap_Cvar_VariableIntegerValue( "g_mSlapPowerZ" );
	time = trap_Cvar_VariableIntegerValue( "g_mSlapKnockDownTime" );
	totaltime = time * 1000;
...
void M_Svcmd_Terminator_f( gentity_t * targetplayer )
{
	...
	targetplayer->client->ps.stats[STAT_WEAPONS] =	( 1 << WP_STUN_BATON | 1 << WP_MELEE | 1 << WP_BRYAR_PISTOL | 1 << WP_BLASTER | 
													1 << WP_DISRUPTOR | 1 << WP_BOWCASTER | 1 << WP_REPEATER | 1 << WP_DEMP2 | 
													1 << WP_FLECHETTE | 1 << WP_ROCKET_LAUNCHER | 1 << WP_THERMAL | 1 << WP_TRIP_MINE | 
		...
		switch(g_mWarningPunishment.integer){
			...
			case 1: //kick
				...
				G_LogPrintf( "mlog_warning_kicked: %s\n with IP %i.%i.%i.%i has met or exceeded the maximum number of warnings.\n", targetplayer->client->pers.netname,
					targetplayer->client->sess.ip[0], targetplayer->client->sess.ip[1], targetplayer->client->sess.ip[2], targetplayer->client->sess.ip[3]);
				break;
			case 2: //kickban
				//send messages
				trap_Cvar_VariableStringBuffer( "g_mMessageWarningKickedTarget", targetString, MAX_TOKEN_CHARS);
				...
				trap_Cvar_VariableStringBuffer( "g_mMessageWarningKickedEveryone", everyoneString, MAX_TOKEN_CHARS);
				//execute commands
				trap_SendConsoleCommand( EXEC_APPEND, va( "AddIP %i.%i.%i.%i", targetplayer->client->sess.ip[0], targetplayer->client->sess.ip[1], targetplayer->client->sess.ip[2], 
					targetplayer->client->sess.ip[3]));
				trap_SendConsoleCommand( EXEC_NOW, va("clientkick %i\n", targetplayer->client->ps.clientNum) );
				//log it
			...
			default: // do nothing special.
				...
				targetplayer->client->sess.ip[0], targetplayer->client->sess.ip[1], targetplayer->client->sess.ip[2], targetplayer->client->sess.ip[3], warnTotal);
				break;
		}
	}
	else{
