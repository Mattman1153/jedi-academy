...
void G_CheapWeaponFire(int entNum, int ev)
{
	...
	switch (ev)
	{
		case EV_FIRE_WEAPON:
			...
			{ //a speeder with a pilot
				...
				{ //pilot is valid...
                    if (rider->client->ps.weapon != WP_MELEE && 
						/* Fix: ensiform (rider->client->ps.weapon != WP_SABER || !rider->client->ps.saberHolstered))*/
			...
			ent->client->pers.protect = qfalse; // MJN - No longer protected
...
void G_SetTauntAnim( gentity_t *ent, int taunt )
{
	...
	// allow some of the taunts on the run
	if ((ent->client->ps.velocity[2] != 0 || ent->client->pers.cmd.upmove || 
		ent->client->pers.cmd.forwardmove || ent->client->pers.cmd.rightmove) &&
		((taunt >= TAUNT_BOW && taunt <= TAUNT_GLOAT)))
	{
		return;
	}
	...
	{//normal taunt always allowed
		...
		// MJN - commented for the new system below
		...
		// MJN - Added switch to allow emotes in preferred circumstances.
		switch( g_mAllowEmotes.integer )
		{
						// Taunts in FFA only, not allowed in Private Duels
			case 1:		if( g_gametype.integer != GT_FFA )
							return;
						else
							if( ent->client->ps.duelInProgress )
								return;
						break;
						// Taunts in Private Duels only
			case 2:		if( !ent->client->ps.duelInProgress )
							return;
						break;
						// Taunts in FFA and Private Duels only
			case 3:		if( g_gametype.integer != GT_FFA)
							return;
						break;
						// Taunts not allowed in Private duels, but everywhere else.
			case 4:		if( ent->client->ps.duelInProgress )
							return;
						break;
						// basejk - Taunts in Duel Gametypes only
			default:	if ( g_gametype.integer != GT_DUEL && g_gametype.integer != GT_POWERDUEL )
							return;
						break;
		}
	}
	...
	if ((ent->client->saberKnockedTime > level.time || 
		ent->client->ps.saberInFlight == qtrue ||
		ent->client->ps.saberEntityNum == 0) && taunt != TAUNT_TAUNT)
	{
		return;
	}
	// end of xMod
		...
		&& ent->client->ps.saberLockTime < level.time )
	{
		...
		if ( anim != -1 )
		{
			...
			} // MJN - Added taunt check for new ones.
			if ( taunt != TAUNT_MEDITATE && taunt != TAUNT_BOW &&
				(taunt < 5 || taunt > MAX_EMOTES ) )
...
//		G_Printf("serverTime >>>>>\n" );
	...
	// MJN - Shall we protect him?
	if (ent && ent->client && (ent->client->ps.eFlags & EF_INVULNERABLE && !M_isProtected(ent)))
	{
...
#define	MIN_NPC_SPEED	16.0f
	...
	if (ent->client->ps.duelInProgress)
	{
		...
		// MJN - Now configurable as if you want to allow a timer between the private dueling. 
		// The default for g_mDuelTimer is 0 (off) and the default for g_mDuelTimerWait is 10 seconds(10000 ms).
		if(g_mDuelTimer.integer){
			ent->client->ps.fd.privateDuelTime = level.time + g_mDuelTimerWait.integer;
		}
		...
		if (ent->client->ps.duelTime < level.time)
		{
				...
				&& ent->client->ps.duelTime )
			{
				...
				// MJN - Save HP and Armor //ent
				M_SaveHPandArmor(ent, g_mPlayerDuelShield.integer);
			}
				...
				&& duelAgainst->client->ps.duelTime)
			{
				...
				// MJN - Holster those sabers! //duelAgainst
				M_HolsterThoseSabers( duelAgainst );

				G_AddEvent(duelAgainst, EV_PRIVATE_DUEL, 2);

				duelAgainst->client->ps.duelTime = 0;

				// MJN - Save HP and Armor //duelAgainst
				M_SaveHPandArmor(duelAgainst, g_mPlayerDuelShield.integer);
			}
		}
		else
		{
		...
		// MJN - This may fix or lessen the likelyhood of the unexpected duel ties all the time.  
		// Now we're checking if they're truly dead, not partially alive, by any means, before we end the duel.
		//else if (duelAgainst->health < 1 || duelAgainst->client->ps.stats[STAT_HEALTH] < 1)
		else if (duelAgainst->health <= 0 && duelAgainst->client->ps.stats[STAT_HEALTH] <= 0)
		{
			//	RMH - Get the end of the duel time.
			char time[MAX_NETNAME] = {0};
			ent->client->pers.duelEndTime = level.time;
			duelAgainst->client->pers.duelEndTime = level.time;

			ent->client->ps.duelInProgress = 0;
			duelAgainst->client->ps.duelInProgress = 0;

			// RMH - Get the duel time.
			if ( g_mDuelStats.integer ) {
				R_GetDuelTime( ent->client->pers.duelEndTime, ent->client->pers.duelStartTime, time );
			}
			...
			{
				// MJN - Dueling clients and / or All can see Duel Stats	
				// RMH - Added Dual Lasted: %s minutes. with %s = time.
				switch( g_mDuelStats.integer ){
						case 1: //Private Logging
								trap_SendServerCommand( ent-g_entities, va("print \"%s ^7defeated %s ^7in ^5%s ^7minutes ^7with ^1%i ^7HP ^7and ^2%i ^7Shields Remaining.\n\"", 
									ent->client->pers.netname, duelAgainst->client->pers.netname, time, ent->client->ps.stats[STAT_HEALTH], ent->client->ps.stats[STAT_ARMOR]) );
								trap_SendServerCommand( duelAgainst-g_entities, va("print \"%s ^7defeated %s ^7in ^5%s ^7minutes ^7with ^1%i ^7HP ^7and ^2%i ^7Shields Remaining.\n\"", 
									ent->client->pers.netname, duelAgainst->client->pers.netname, time, ent->client->ps.stats[STAT_HEALTH], ent->client->ps.stats[STAT_ARMOR]) );
								break;
						case 2: //Public Logging
								trap_SendServerCommand( -1, va("print \"%s ^7defeated %s ^7in ^5%s ^7minutes ^7with ^1%i ^7HP ^7and ^2%i ^7Shields Remaining.\n\"", 
									ent->client->pers.netname, duelAgainst->client->pers.netname, time, ent->client->ps.stats[STAT_HEALTH], ent->client->ps.stats[STAT_ARMOR] ) );
								break;
						default: // MJN - Do it the basejk way
				...
				// MJN - Get Saved HP and Armor Back
				M_LoadHPandArmor(ent, g_mPlayerDuelShield.integer);
				M_SetDuelForcePool(ent, g_mDuelForce.integer);
				...
				// MJN - It's a tie, we're both dead!!! Dueling clients and / or All can see Duel Stats
				switch( g_mDuelStats.integer ){
						case 1: //Private Logging
		...
		else
		{
			...
			if (subLen >= 1024)
			{
				...
				{
					M_LoadHPandArmor(duelAgainst, g_mPlayerDuelShield.integer);
					M_SetDuelForcePool(duelAgainst, g_mDuelForce.integer);
				}
				// MJN - WTF it stopped!!! the duel ended for other reason than a winner...
				switch( g_mDuelStats.integer ){
						case 1: //Private Logging
								trap_SendServerCommand( ent-g_entities, va("print \"%s\n\"", G_GetStringEdString("MP_SVGAME", "PLDUELSTOP")) );
								trap_SendServerCommand( duelAgainst-g_entities, va("print \"%s\n\"", G_GetStringEdString("MP_SVGAME", "PLDUELSTOP")) );
								break;
						case 2: //Public Logging
...
//	G_VehicleAttachDroidUnit( ent );
	...
	// MJN - Perform admin mod actions:
	M_Cmd_ClientActions_f( ent );
