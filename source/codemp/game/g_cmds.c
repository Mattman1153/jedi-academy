...
void Cmd_Kill_f( gentity_t *ent ) {
	...
	if(M_isSleeping(ent))
	{
		trap_SendServerCommand( ent-g_entities, va("print \"Killing yourself will not solve anything.\n\"") );
		return;
	}
	// MJN - for keeping duels straight up SVINGAME
	if (!g_allowDuelSuicide.integer && ent->client->ps.duelInProgress)
	{
	...
	// MJN - to prevent team killing to alter scores
	if (!g_mAllowTeamSuicide.integer && g_gametype.integer == GT_TEAM || g_gametype.integer == GT_SIEGE
		|| g_gametype.integer == GT_CTF || g_gametype.integer == GT_CTY)
	{
			trap_SendServerCommand( ent-g_entities, va("print \"Suicides are disabled for this gametype.\n\"") );
			return;
	}
	ent->flags &= ~FL_GODMODE;
...
void Cmd_Team_f( gentity_t *ent ) {
	...
	// MJN - Can't switch teams when in private duel
	if ( g_mAllowTeamDuel.integer && ent->client->ps.duelInProgress )
	{ //don't let clients change teams manually at all in powerduel, it will be taken care of through automated stuff
		trap_SendServerCommand( ent-g_entities, "print \"Cannot switch teams when in a private duel\n\"" );
		return;
	}

	// MJN - Explicit calls for Empowered Vs. Terminator
	if(g_trueJedi.integer && g_mJvsT.integer){

		if(M_isEmpowered(ent) && ent->client->ps.trueJedi){
			M_Svcmd_UnEmpower_f(ent);
		}else if(M_isTerminator(ent) && ent->client->ps.trueNonJedi){
			M_Svcmd_UnTerminator_f(ent);
		}
	}
	else{
		// MJN - Undo those extra powers!
		if(M_isEmpowered(ent)){
			M_Svcmd_UnEmpower_f(ent);
		}
		if(M_isProtected(ent)){
			M_Svcmd_UnProtect_f(ent);
		}
...
void Cmd_FollowCycle_f( gentity_t *ent, int dir ) {
	...
	do {
		...
		// MJN - Cannot follow an admin.
		if ( g_mNoAdminFollow.integer && (level.clients[ clientnum ].sess.loggedinadmin || 
				level.clients[ clientnum ].sess.loggedinadmin ) == qtrue ) {
			continue;
		}
...
static void G_SayTo( gentity_t *ent, gentity_t *other, int mode, int color, const char *name, const char *message, char *locMsg )
{
	...
	// MJN - No chatting to players in tournements
	if ( g_mSTFU.integer && (g_gametype.integer == GT_DUEL || g_gametype.integer == GT_POWERDUEL)
...
void G_Say( gentity_t *ent, gentity_t *target, int mode, const char *chatText ) {
	...
	switch ( mode ) {
		...
		case SAY_ALL:
			...
			if (g_mRankDisplay.integer && (M_isAdmin(ent) || M_isKnight(ent))){
				Com_sprintf (name, sizeof(name), "%s%c%c(%s)"EC": ", ent->client->pers.netname, Q_COLOR_ESCAPE, COLOR_WHITE, M_GetPlayerRank(ent) );
			}
			else{
				Com_sprintf (name, sizeof(name), "%s%c%c"EC": ", ent->client->pers.netname, Q_COLOR_ESCAPE, COLOR_WHITE );
			}
			//color = COLOR_GREEN;
			color = M_SetConsoleTextColor(color, g_mSayChatColor.integer);
			break;
		case SAY_TEAM:
			...
			if (Team_GetLocationMsg(ent, location, sizeof(location)))
			{
				if (g_mRankDisplay.integer && (M_isAdmin(ent) || M_isKnight(ent))){
					Com_sprintf (name, sizeof(name), EC"(%s%c%c(%s)"EC")"EC": ", 
						ent->client->pers.netname, Q_COLOR_ESCAPE, COLOR_WHITE, M_GetPlayerRank(ent) );
				}
				else{
			...
			//color = COLOR_CYAN;
			color = M_SetConsoleTextColor(color, g_mTeamChatColor.integer);
			break;
		case SAY_TELL:
			...
			{
				if (g_mRankDisplay.integer && (M_isAdmin(ent) || M_isKnight(ent))){
					Com_sprintf (name, sizeof(name), EC"[%s%c%c(%s)"EC"]"EC": ", ent->client->pers.netname, Q_COLOR_ESCAPE, COLOR_WHITE, M_GetPlayerRank(ent) );
				}
				else{
			...
			//color = COLOR_MAGENTA;
			color = M_SetConsoleTextColor(color, g_mTellChatColor.integer);
			break;
		}
	...
	// MJN - silenced
	if ( M_isSilenced(ent) ) {
		trap_SendServerCommand( ent->client->ps.clientNum, "cp \"You have been silenced and can't talk!\"" );
		return;
	}
	if ( target ) {
	...
	if ( M_isPrivateChatOnly(ent) ) {
		trap_SendServerCommand( ent->client->ps.clientNum, "cp \"You can only use Private Chat!\"" );
		return;
	}
	...
	if ( strstr( text, "!rules" ) || strstr( text, "!motd") ){
		trap_SendConsoleCommand( EXEC_APPEND, va( "mshowmotd %i", ent->client->ps.clientNum ) );
		return;
	}

	if ( strstr( text, "!version" ) ||  strstr( text, "!author") ){
		trap_SendServerCommand( ent->client->ps.clientNum, va("print \"^4%s ^7by ^4%s^7\n\"", GAMEVERSION, AUTHOR));
		return;
	}

	if ( strstr( text, "!home" ) ||  strstr( text, "!web") ){
		trap_SendServerCommand( ent->client->ps.clientNum, "print \"^5http://jamod.jk3files.com^7\n\"" );
		return;
	}
	if ( strstr( text, "!license" ) ||  strstr( text, "!about") ){
		trap_SendServerCommand( ent->client->ps.clientNum, va("print \"^4%s, Copyright (C) 2003 - 2007 - %s^7\n\"", GAMEVERSION, AUTHOR) );
		trap_SendServerCommand( ent->client->ps.clientNum, "print \"The mod comes with ABSOLUTELY NO WARRANTY; This is free software,\n\""); 
		trap_SendServerCommand( ent->client->ps.clientNum, "print \"and you are welcome to redistribute it under certain conditions;\n\"" );
		trap_SendServerCommand( ent->client->ps.clientNum, "print \"See packaged document for further details.^7\n\"" );
		return;
	}

	// RMH - using @ routes text to amsay with normal chat.
	// MJN - the ported code is not working.  I deleted it and I am leaving it just for amsay.
	temp = strstr( text, "@ " );
	temp2 = strstr( text, "amsay " );
	temp3 = strstr( text, "?amsay " );
	
	if ( temp &&  (strncmp( temp, text, strlen( temp ) )) == 0 ) {
		M_Cmd_ASay_f( ent );
		return;
	}
	// MJN - Added two additional ways to capture amsay as typos can occur and cause information to go to the wrong clients.
	if ( temp2 &&  (strncmp( temp2, text, strlen( temp2 ) )) == 0 ) {
		M_Cmd_ASay_f( ent );
		return;
	}
	if ( temp3 &&  (strncmp( temp3, text, strlen( temp3 ) )) == 0 ) {
		M_Cmd_ASay_f( ent );
		return;
	}
...
static void Cmd_Tell_f( gentity_t *ent ) {
	...
	// MJN - So we can pm using names.
	//targetNum = atoi( arg );
	targetNum = M_G_ClientNumberFromName( arg );
...
//	char*	type = NULL;
	...
	// MJN - mallowvote/mdenyvote
	if ( !M_isAllowedVote(ent) ){
		trap_SendServerCommand( ent->client->ps.clientNum, va("print \"^1Admin has disabled use of your voting privilege!\n\""));
		trap_SendServerCommand( ent->client->ps.clientNum, va("cp \"^3Admin ^7has disabled use of your\n voting privilege!\n\""));
		return;
	}
...
#ifndef FINAL_BUILD
	// MJN - Added check for g_saberDebugPrint here so we wouldn't lose this functionality in Debug mode.
...
void Cmd_EngageDuel_f(gentity_t *ent)
{
	...
	// MJN - Are we going to allow private duels?
	if ( !g_mAllowTeamDuel.integer && g_gametype.integer >= GT_TEAM )
	...
	//lmo check for overtime
	if ( duel_overtime == qtrue )
	{ // tell player
		trap_SendServerCommand(ent-g_entities, va("print \"Overtime period, no new duels allowed.\n\""));
		return;
	}
	...
	if (InEmote(ent->client->emote_num ) || 
		InSpecialEmote(ent->client->emote_num )){
		G_SetTauntAnim(ent, ent->client->emote_num);
	}

	//New: Don't let a player duel if he just did and hasn't waited 10 seconds yet (note: If someone challenges him, his duel timer will reset so he can accept)
	// MJN - Now uses a Cvar to determine if we want this to work.  My as well give you the option to keep it like basejk. ;)
	if (g_mDuelTimer.integer && ent->client->ps.fd.privateDuelTime > level.time)
	{
		//trap_SendServerCommand( ent-g_entities, va("print \"%s\n\"", G_GetStringEdString("MP_SVGAME", "CANTDUEL_JUSTDID")) );
		// MJN - This is the same exact message, but I needed to display the amount of time dynamically.
		trap_SendServerCommand( ent-g_entities, va("print \"You must wait %i seconds after finishing a duel to initiate another duel, or have someone else challenge you.\n\"", g_mDuelTimerWait.integer / 1000) );
		return;
	}
	// // MJN - Do we allow to duel if other are?
	if (g_mEnforceDuel.integer && G_OtherPlayersDueling())
	{
	...
	if (tr.fraction != 1 && tr.entityNum < MAX_CLIENTS)
	{
		...
		// MJN - added friendly fire check if you are on the same team as the one you challenged.
		if ( g_gametype.integer >= GT_TEAM && ( !g_friendlyFire.integer && OnSameTeam(ent, challenged))){
			return;
		}
		
		// MJN - We don't want them to challenge the protected 
		// and you cannot challenge others if you are protected.
		if ( M_isProtected(ent) || M_isProtected(challenged) ){
			return;
		}
		
		// MJN - Can only duel Flag Carrier if you are a Flag Carrier in CTF
		if ( g_gametype.integer == GT_CTF ){
			if ( ent->client->ps.powerups[PW_REDFLAG] && !challenged->client->ps.powerups[PW_BLUEFLAG]){
				return;
			}else if ( ent->client->ps.powerups[PW_BLUEFLAG] && !challenged->client->ps.powerups[PW_REDFLAG]){
				return;
			}else if ( !ent->client->ps.powerups[PW_REDFLAG] && challenged->client->ps.powerups[PW_BLUEFLAG]){
				return;
			}else if ( !ent->client->ps.powerups[PW_BLUEFLAG] && challenged->client->ps.powerups[PW_REDFLAG]){
				return;
			}
		...
		if (challenged->client->ps.duelIndex == ent->s.number && challenged->client->ps.duelTime >= level.time)
		{
			...
			// RMH - Init these vars.
			ent->client->pers.duelEndTime = 0;
			challenged->client->pers.duelEndTime = 0;
			...
			M_HolsterThoseSabers(challenged);
		}
		else
		{
		...
		// MJN - If this is enabled...
		if(g_mDuelTimer.integer){
...
qboolean TryGrapple(gentity_t *ent)
{
	...
	// TODO: MJN - may need to add code here to check if we're protected and release the flag.
...
void ClientCommand( int clientNum ) {
	...
	{
		if(ent->client->pers.connected && M_isNPCAccess(ent)){
			Cmd_NPC_f( ent );
		}
...
// MJN - Commented this out because I do not want the below commands available even if cheats are enabled.
//#endif
