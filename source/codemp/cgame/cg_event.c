	...
	case EV_PRIVATE_DUEL:
		...
		{ //starting the duel
			...
			else
			{
				...
				if(cg_mAllowMultiDuelMusic.integer){
					// MJN - Lets play some random duel music.
					char *music;
					int randNum;
					// Get maximum files I want to randomly play
					randNum = Q_irand( 0, cg_mDuelMusicFiles.integer );
					
					if ( randNum > 0 ){
						
						// Initialize the music file.
						music = va("music/mp/duel%d.mp3", randNum);
						// We want to make sure that the start track is the same as the loop track.
						trap_S_StartBackgroundTrack( music, music, qfalse );
					}
					else{
						//we still want to play the basejk track.
